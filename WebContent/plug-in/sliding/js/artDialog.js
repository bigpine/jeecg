...
(function(l, g) {
	...
	}, m = false, p = [], n, i, c = "opacity" in document.documentElement.style, b = /^(?:[^<]*(<[\w\W]+>)[^>]*$|#([\w\-]+)$)/, f = /[\n\t]/g, h = /alpha\([^)]*\)/i, q = /opacity=([^)]*)/, e = /^([+-]=)?([\d+-.]+)(.*)$/;
	if (l.$ === g) {
		l.$ = k
	}
	k.fn = k.prototype = {
		...
		offset : function() {
			var t = this[0], v = t.getBoundingClientRect(), z = t.ownerDocument, w = z.body, r = z.documentElement, u = r.clientTop
					|| w.clientTop || 0, x = r.clientLeft || w.clientLeft || 0, y = v.top
					+ (self.pageYOffset || r.scrollTop) - u, s = v.left
					+ (self.pageXOffset || r.scrollLeft) - x;
			return {
				left : s,
				top : y
			}
	...
	function o(x, s, z) {
		s = s || document;
		z = z || "*";
		var v = 0, u = 0, y = [], t = s.getElementsByTagName(z), r = t.length, w = new RegExp(
				"(^|\\s)" + x + "(\\s|$)");
		for (; v < r; v++) {
	...
	k.removeData = function(t, s) {
		var v = true, y = k.expando, r = k.cache, x = a(t), u = x && r[x];
		if (!u) {
			return
		}
		if (s) {
	...
	k.event = {
		add : function(v, t, x) {
			var r, s, u = k.event, w = k.data(v, "@events")
					|| k.data(v, "@events", {});
			r = w[t] = w[t] || {};
			...
			if (!r.handler) {
				...
				document.addEventListener ? v.addEventListener(t, r.handler,
						false) : v.attachEvent("on" + t, r.handler)
			}
		},
		remove : function(t, y, A) {
			var v, r, z, x = k.event, w = true, u = k.data(t, "@events");
			if (!u) {
				return
			}
			if (!y) {
			...
			if (r.listeners.length === 0) {
				document.removeEventListener ? t.removeEventListener(y,
						r.handler, false) : t.detachEvent("on" + y, r.handler);
				delete u[y];
				r = k.data(t, "@events");
				for ( var s in r) {
					w = false
	...
	k.cleanData = function(s) {
		var t = 0, u, r = s.length, v = k.event.remove, w = k.removeData;
		for (; t < r; t++) {
			u = s[t];
			v(u);
			w(u)
	...
	k.css = "defaultView" in document
			&& "getComputedStyle" in document.defaultView ? function(s, r) {
	...
	k.opacity = {
		get : function(r) {
			return c ? document.defaultView.getComputedStyle(r, false).opacity
					: q.test((r.currentStyle ? r.currentStyle.filter
							: r.style.filter)
		...
		set : function(u, v) {
			...
			var t = u.style;
			t.zoom = 1;
			var r = "alpha(opacity=" + v * 100 + ")", s = t.filter || "";
			t.filter = h.test(s) ? s.replace(h, r) : t.filter + " " + r
		}
	};
	k.each([ "Left", "Top" ], function(s, r) {
		var t = "scroll" + r;
		k.fn[t] = function(w) {
			var u = this[0], v;
			v = d(u);
			return v ? ("pageXOffset" in v) ? v[s ? "pageYOffset"
					: "pageXOffset"] : v.document.documentElement[t]
					|| v.document.body[t] : u[t]
		}
	});
	function d(r) {
		return k.isWindow(r) ? r : r.nodeType === 9 ? r.defaultView
				|| r.parentWindow : false
	}
	k.each([ "Height", "Width" ], function(s, r) {
		var t = r.toLowerCase();
		k.fn[t] = function(u) {
			var v = this[0];
			if (!v) {
			...
			return k.isWindow(v) ? v.document.documentElement["client" + r]
					|| v.document.body["client" + r]
					: (v.nodeType === 9) ? Math.max(v.documentElement["client"
							+ r], v.body["scroll" + r],
							v.documentElement["scroll" + r], v.body["offset"
									+ r], v.documentElement["offset" + r])
							: null
		}
	});
	k.ajax = function(t) {
		var v = l.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject(
				"Microsoft.XMLHTTP"), s = t.url;
		if (t.cache === false) {
			var u = (new Date()).getTime(), r = s.replace(/([?&])_=[^&]*/,
					"$1_=" + u);
			s = r + ((r === s) ? (/\?/.test(s) ? "&" : "?") + "_=" + u : "")
		}
	...
	k.fn.animate = function(r, u, z, C) {
		...
		s.curAnim = {};
		k.each(r, function(D, E) {
			s.curAnim[D] = E
		});
		k.each(r, function(D, E) {
			w = new k.fx(A[0], s, D);
			y = e.exec(E);
			t = parseFloat(D === "opacity"
					|| (A[0].style && A[0].style[D] != null) ? k.css(A[0], D)
					: A[0][D]);
			x = parseFloat(y[2]);
			B = y[3];
			if (D === "height" || D === "width") {
				x = Math.max(0, x);
				v = [ A[0].style.overflow, A[0].style.overflowX,
						A[0].style.overflowY ]
			}
			w.custom(t, x, B)
		});
	...
	k.fx.prototype = {
		...
		step : function() {
			...
			if (u >= v.options.speed + v.startTime) {
				...
				v.pos = k.easing[v.options.easing](v.state, w, 0, 1,
						v.options.speed);
...
(function(d, m, h) {
	...
	var o, b, k, c, s = 0, t = d(m), i = d(document), g = d("html"), f = d(function() {
		f = d("body")
	}), n = document.documentElement, a = m.VBArray && !m.XMLHttpRequest, q = "createTouch" in document
			&& !("onmousemove" in n)
			|| /(iPhone|iPad|iPod)/i.test(navigator.userAgent), p = "artDialog"
			+ (new Date).getTime();
	var l = function(e, v, z) {
		...
		var w, y = [], A = l.defaults, x = e.follow = this.nodeType === 1
				&& this || e.follow;
		for ( var u in A) {
			if (e[u] === h) {
				e[u] = A[u]
	...
	l.fn = l.prototype = {
		version : "4.1.2",
		_init : function(e) {
			var w = this, v, u = e.icon, x = u
					&& (a ? {
						png : "icons/" + u + ".png"
					} : {
						backgroundImage : "url('" + e.path + "/skins/icons/"
								+ u + ".png')"
					});
			w._isRun = true;
			w.config = e;
			...
			w.button(e.button).title(e.title).content(e.content, true).size(
					e.width, e.height).time(e.time);
		...
		position : function(A, G) {
			var F = this, y = F.config, v = F.DOM.wrap[0], B = a ? false
					: y.fixed, E = a && F.config.fixed, z = i.scrollLeft(), I = i
					.scrollTop(), D = B ? 0 : z, w = B ? 0 : I, C = t.width(), u = t
					.height(), x = v.offsetWidth, H = v.offsetHeight, e = v.style;
			if (A || A === 0) {
		...
		follow : function(O) {
			...
			var A = p + "follow", F = t.width(), v = t.height(), x = i
					.scrollLeft(), z = i.scrollTop(), y = e.offset(), K = O.offsetWidth, J = O.offsetHeight, B = a ? false
					: P.fixed, w = B ? y.left - x : y.left, H = B ? y.top - z
					: y.top, D = C.DOM.wrap[0], N = D.style, u = D.offsetWidth, M = D.offsetHeight, E = w
					- (u - K) / 2, I = H + J, L = B ? 0 : x, G = B ? 0 : z;
		...
		button : function() {
			var w = this, u = arguments, A = w.DOM, e = A.wrap, y = A.buttons, v = y[0], B = "aui_state_highlight", z = w._listeners = w._listeners
					|| {}, x = d.isArray(u[0]) ? u[0] : [].slice.call(u);
			if (u[0] === h) {
				return v
			}
			d.each(x, function(E, G) {
				var C = G.name, F = !z[C], D = !F ? z[C].elem : document
		...
		close : function() {
			...
			var y = this, x = y.DOM, w = x.wrap, z = l.list, v = y.config.close, e = y.config.follow;
			y.time();
		...
		focus : function() {
			var A, x = this, w = x.DOM, v = w.wrap, z = l.focus, u = l.defaults.zIndex++;
			v.css("zIndex", u);
		...
		lock : function() {
			...
			var y = this, z = l.defaults.zIndex - 1, v = y.DOM.wrap, x = y.config, A = i
					.width(), D = i.height(), B = y._lockMaskWrap
					|| d(f[0].appendChild(document.createElement("div"))), w = y._lockMask
					|| d(B[0].appendChild(document.createElement("div"))), u = "(document).documentElement", e = q ? "width:"
					+ A + "px;height:" + D + "px"
					: "width:100%;height:100%", C = a ? "position:absolute;left:expression("
					+ u
					+ ".scrollLeft);top:expression("
					+ u
					+ ".scrollTop);width:expression("
					+ u
					+ ".clientWidth);height:expression(" + u + ".clientHeight)"
					: "";
			y.focus(true);
			v.addClass("aui_state_lock");
			B[0].style.cssText = e + ";position:fixed;z-index:" + z
					+ ";top:0;left:0;overflow:hidden;" + C;
			w[0].style.cssText = "height:100%;background:" + x.background
					+ ";filter:alpha(opacity=0);opacity:0";
			if (a) {
				w
						.html('<iframe src="about:blank" style="width:100%;height:100%;position:absolute;top:0;left:0;z-index:-1;filter:alpha(opacity=0)"></iframe>')
			}
			w.stop();
		...
		_getDOM : function() {
			...
			var u, w = 0, y = {
				wrap : d(x)
			}, v = x.getElementsByTagName("*"), e = v.length;
			for (; w < e; w++) {
		...
		_ie6PngFix : a ? function() {
			var u = 0, w, z, v, e, y = l.defaults.path + "/skins/", x = this.DOM.wrap[0]
			...
			for (; u < x.length; u++) {
				...
				if (z) {
					...
					e.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
							+ v + "',sizingMethod='crop')"
				}
			}
		}
				: d.noop,
		_ie6SelectFix : a ? function() {
			var u = this.DOM.wrap, x = u[0], y = p + "iframeMask", w = u[y], v = x.offsetWidth, e = x.offsetHeight;
			v = v + "px";
			e = e + "px";
			if (w) {
				w.style.width = v;
				...
				u[y] = w;
				w.src = "about:blank";
				w.style.cssText = "position:absolute;z-index:-1;left:0;top:0;filter:alpha(opacity=0);width:"
						+ v + ";height:" + e
			}
		}
				: d.noop,
		...
		_setFixed : (function() {
			...
			return function() {
				...
				if (a) {
					var z = parseInt(w.css("left")), y = parseInt(w.css("top")), v = i
							.scrollLeft(), u = i.scrollTop(), e = "(document.documentElement)";
					this._setAbsolute();
					x.setExpression("left", "eval(" + e + ".scrollLeft + "
							+ (z - v) + ') + "px"');
					x.setExpression("top", "eval(" + e + ".scrollTop + "
							+ (y - u) + ') + "px"')
				} else {
					x.position = "fixed"
				}
		...
		_click : function(e) {
			...
			return typeof u !== "function" || u.call(v, m) !== false ? v
					.close() : v
		},
		_reset : function(z) {
		...
		_addEvent : function() {
			var e, x = this, u = x.config, v = "CollectGarbage" in m, w = x.DOM;
	...
	i
			.bind(
					"keydown",
					function(v) {
						var x = v.target, y = x.nodeName, e = /^INPUT|TEXTAREA$/, u = l.focus, w = v.keyCode;
	...
	c = m._artDialog_path
			|| (function(e, u, v) {
				...
				b = v || e[e.length - 1];
				v = b.src.replace(/\\/g, "/");
				return v.lastIndexOf("/") < 0 ? "." : v.substring(0, v
						.lastIndexOf("/"))
...
(function(e) {
	var h, b, a = e(window), d = e(document), i = document.documentElement, f = !-[
			1, ]
			&& !("minWidth" in i.style), g = "onlosecapture" in i, c = "setCapture" in i;
	...
	b = function(j) {
		var o, p, w, l, s, u, r = artDialog.focus, m = r.config, x = r.DOM, k = x.wrap, t = x.title, n = x.main, q = m.width;
		relheight = m.height;
		var v = "getSelection" in window ? function() {
		...
		h.onstart = function(z, A) {
			...
			!f && g ? t.bind("losecapture", h.end) : a.bind("blur", h.end);
			c && t[0].setCapture();
		...
		h.onmove = function(A, G) {
			if (u) {
				var D = k[0].style, C = n[0].style, B = A + p, z = G + w;
				D.width = "auto";
				C.width = B + "px";
				D.width = k[0].offsetWidth + "px";
				C.height = Math.max(0, z) + "px"
			} else {
				var C = k[0].style, F = A + l, E = G + s;
		...
		h.onend = function(z, A) {
			...
			!f && g ? t.unbind("losecapture", h.end) : a.unbind("blur", h.end);
		...
		o = (function() {
			var z, y, B = r.DOM.wrap[0], E = B.style.position === "fixed", D = B.offsetWidth, H = B.offsetHeight, F = a
					.width(), A = a.height(), G = E ? 0 : d.scrollLeft(), C = E ? 0
					: d.scrollTop(), z = F - D + G;
			y = A - H + C;
			return {
				minX : G,
	...
	d.bind("mousedown", function(m) {
		...
		if (j.drag !== false && n === l.title[0] || j.resize !== false
				&& n === l.se[0]) {
